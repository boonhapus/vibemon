...
def check_pre_action(ctx: turn.Turn, vibemon: battle_schema.BattleVibemon) -> PreActionResult:
    ...
    if vibemon.status == types.StatusConditionT.SLEEP:
        vibemon.sleep_turns_remaining -= 1
        if vibemon.sleep_turns_remaining <= 0:
            vibemon.status = types.StatusConditionT.NONE
    ...
    if vibemon.status == types.StatusConditionT.FREEZE:
    ...
    if vibemon.status == types.StatusConditionT.PARALYSIS and ctx.rng.random() < const.PARALYSIS_FULLY_PARALYZED_CHANCE:
        return PreActionResult(
            blocked=True,
...
def apply_status_damage(vibemon: battle_schema.BattleVibemon) -> int:
...
def end_of_turn_maintenance(vibemon: battle_schema.BattleVibemon) -> list[event_models.TurnEvent]:
