...
from app.domains.battle import const, entity, events, turn
from app.domains.battle.mechanics import stats
from app.domains.move import catalog
from app.domains.move.types import MoveCategoryT, StatusConditionT
...
def calc_damage(
    ...
    *,
    ...
    effectiveness = catalog.get_element_effectiveness(move.type, defender.elements)
    if effectiveness == 0:
        return events.DamageResult(
            damage=0,
            is_crit=is_crit,
    ...
    if attacker.status == StatusConditionT.BURN and move.category == MoveCategoryT.PHYSICAL:
        ordered.append(
            (
                "burn",
