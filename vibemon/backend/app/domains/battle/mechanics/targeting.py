...
from app.domains.move.types import MoveTargetT


def trainer_for_id(ctx: turn.Turn, trainer_id: TrainerIdT) -> entity.BattleTrainer:
...
def opposing_trainer(ctx: turn.Turn, trainer_id: TrainerIdT) -> entity.BattleTrainer:
...
def resolve_targets(ctx: turn.Turn, use: turn.MoveUse) -> tuple[entity.BattleVibemon, ...]:
    """Resolve move targets for the current 1v1 topology."""
    move = use.move
    if move.target in (MoveTargetT.SELF, MoveTargetT.USER_SIDE):
        return (use.user,)
    if move.target == MoveTargetT.FIELD:
        return ()
