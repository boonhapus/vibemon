...
from app.core.errors import GenerationAlreadyActive, GenerationCreditUnavailable, PartyFull
...
def test_party_selects_first_open_slot() -> None:
    assert party.select_adoption_slot(owned_count=3, used_slots={0, 2, 3}, release_slot=None) == 1


def test_party_requires_release_when_full() -> None:
    with pytest.raises(PartyFull):
        party.select_adoption_slot(owned_count=party.MAX_PARTY_SIZE, used_slots=set(range(6)), release_slot=None)


def test_party_uses_release_slot_when_full() -> None:
    assert party.select_adoption_slot(owned_count=party.MAX_PARTY_SIZE, used_slots=set(range(6)), release_slot=4) == 4
